...
export default function Bookings({ bookings, onAddBooking, onCancelBooking, userDetails }) {
  ...
  const [serviceCentre, setServiceCentre] = useState('Krishnanagar-Ghurni');
  const [selectedDate, setSelectedDate] = useState('');
  const [customerName, setCustomerName] = useState('');
  const [customerPhone, setCustomerPhone] = useState('');
  const [notes, setNotes] = useState('');
  const [gstBill, setGstBill] = useState(null);
  ...
  const handleBookingSubmit = (e) => {
    ...
    const newBooking = {
      ...
      gstBill,
      pickupFromHome,
      pickupAddress: pickupFromHome ? pickupAddress : '',
      status: 'Scheduled',
    ...
    <div className="animated-page">
      ...
      <>
        ...
        <form className="booking-form-card" onSubmit={handleBookingSubmit}>
          ...
          {/* Optional Purchase GST Bill */}
          <div className="form-group" style={{ marginBottom: '16px' }}>
            <label className="form-label">Purchase GST Bill </label>
            <input
              type="file"
              accept=".jpg,.jpeg,.pdf"
              className="form-input"
          ...
          <div className="form-group" style={{ marginBottom: '16px', flexDirection: 'row', alignItems: 'center', gap: '8px' }}>
            <input
              ...
              onChange={(e) => setPickupFromHome(e.target.checked)}
              style={{ width: '16px', height: '16px', cursor: 'pointer' }}
            />
            <label htmlFor="pickupFromHome" className="form-label" style={{ cursor: 'pointer', margin: 0 }}>Pick up from home</label>
          </div>
